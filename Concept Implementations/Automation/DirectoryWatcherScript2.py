...
def TraverseDirectory(DirectoryPath):
	...
	for DirPath, SubDirPaths, FileNames in Result:
		...
		for FileName in FileNames:
			# os.path.join(DirPath, FileName) - to get absolute path of FileName
			# print("File in directory {}:".format(DirPath), FileName,"having size", os.path.getsize(os.path.join(DirPath, FileName)))
			print("File in directory {}:".format(DirPath), FileName,"having size", os.path.getsize(DirPath + "/" + FileName))
