...
def CopyFiles(SrcDirectoryPath, DestDirectoryPath):
	# validate path
	...
	try:
		...
		return "Copying files failed: " + str(ex)

def main():
	FileOutput = None
	if len(argv) != 3:
		FileOutput = "Error: Invalid no. of arguments, script not executed. Sample Correct Usage: DirectoryFilesCopy.py Demo Temp"
	else:
		FileOutput = CopyFiles(argv[1], argv[2])
	
	if FileOutput:
		try:
			FileHandle = open(datetime.datetime.now().strftime("DirectoryFilesCopyOutput_%d-%m-%Y_%H-%M-%S.txt"), "w", encoding = "utf-8")
